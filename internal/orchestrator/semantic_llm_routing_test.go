...
// oracle plugin (oracle.local), map its {intent, confidence} verdict onto the
// confidence bands, and route accordingly — instead of falling through to the
// main-turn LLM. A fake oracle stands in for the local model so the test is
// fast and offline.
...
// stubRoutingOracle records its calls and returns a fixed routing verdict.
type stubRoutingOracle struct {
	calls      int32
	submission string
}

func (s *stubRoutingOracle) Ask(ctx context.Context, req oracle.AskRequest) (oracle.AskResponse, error) {
...
func (s *stubRoutingOracle) Close() error { return nil }
...
// The fake oracle.local returns go_west; the harness (if ever reached) would
...
// falls through to the harness and lands in south_end (and the oracle is never
...
routing:
  ...
  extract_llm_oracle: oracle.local
...
`
	...
	stub := &stubRoutingOracle{submission: `{"intent":"go_west","confidence":0.95}`}
	reg := oracle.NewRegistry()
	reg.Register("oracle.local", stub)
	...
	require.Equal(t, int32(1), atomic.LoadInt32(&stub.calls),
		"the local-model oracle must be called exactly once on a deterministic no_match")
	...
	// match_type names the backend plugin (oracle.local). This is what tells an
	...
	require.Equal(t, "oracle.local", matchType, "TurnStarted must record the backend plugin name")
}
...
`
	...
	stub := &stubRoutingOracle{submission: `{"intent":"none","confidence":0.2}`}
	reg := oracle.NewRegistry()
	reg.Register("oracle.local", stub)
	...
	require.Equal(t, int32(1), atomic.LoadInt32(&stub.calls), "oracle still consulted once")
