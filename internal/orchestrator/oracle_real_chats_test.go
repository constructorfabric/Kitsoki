...
//
// Sequence:
//  1. main → go_oracle  → oracle (on_enter loads chat list — empty)
//  2. oracle → new_chat → oracle_active_new (on_enter: host.chat.create)
//  3. Verify a chat row was created in the DB.
//  4. oracle_active_new → ask_question → oracle_asking (on_enter: oracle.talk stub + suggest_title stub)
//  5. Verify two chat_messages rows (user + assistant) landed in the DB.
//  6. go_back → oracle → list now reports 1 chat.
func TestOracle_E2E_RealChatsStore(t *testing.T) {
	...
	reg.Register("host.oracle.talk", func(ctx context.Context, args map[string]any) (host.Result, error) {
		...
		if cs == nil || chatID == "" {
			return host.Result{Error: "stub-oracle: chat store / chat_id missing"}, nil
		}
	...
	reg.Register("host.oracle.ask", func(ctx context.Context, args map[string]any) (host.Result, error) {
	...
	// Step 1: navigate main → oracle.
	out, err := orch.SubmitDirect(ctx, sid, "go_oracle", map[string]any{})
	require.NoError(t, err)
	require.Equal(t, orchestrator.ModeTransitioned, out.Mode)
	require.Equal(t, app.StatePath("oracle"), out.NewState)

	// At this point the oracle list view fired host.chat.list — should
	...
	// Step 2: new_chat → oracle_active_new (on_enter creates a chat via host.chat.create).
	out, err = orch.SubmitDirect(ctx, sid, "new_chat", map[string]any{
		"title": "End-to-end Test Chat",
	})
	require.NoError(t, err)
	require.Equal(t, orchestrator.ModeTransitioned, out.Mode)
	require.Equal(t, app.StatePath("oracle_active_new"), out.NewState)

	// Step 3: a chat row exists.
	allChats, err := rawChatStore.List(ctx, "dev-story", "oracle", "")
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, orchestrator.ModeTransitioned, out.Mode)
	require.Equal(t, app.StatePath("oracle_asking"), out.NewState)
	...
	// Step 6: go_back → oracle (list should now report 1 chat).
	...
	require.NoError(t, err)
	count, _ := journey.World.Vars["oracle_chat_count"].(int)
	if count == 0 {
		// world coercion: the binding may have come back as int64 / float64.
		if c64, ok := journey.World.Vars["oracle_chat_count"].(int64); ok {
			count = int(c64)
		}
		if cf, ok := journey.World.Vars["oracle_chat_count"].(float64); ok {
			count = int(cf)
		}
	}
	assert.Equal(t, 1, count, "after going back to oracle, list should report 1 chat")
}
