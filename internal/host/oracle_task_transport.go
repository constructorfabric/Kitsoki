// Package host — event-emitting MCP transport shim for host.oracle.task.
//
// The task handler needs to observe every tool call the agent makes so each
// Edit/Write/Bash/sub-oracle call produces a task.tool journal event. This
...
//	kitsoki.oracle.extract, kitsoki.oracle.decide, kitsoki.oracle.ask.
//	Their invocations produce child oracle spans under the parent trace.
//	The implementation here is a hook point; full sub-oracle routing lands
//	in Phase 5/6 when the oracle CLI surface exists.
//
...
//	kitsoki oracle calls join the parent trace. The env var is injected by
