...
// TestOracleAsk_AgentTools_ForwardedAsAllowedTools verifies that when an agent
// declares Tools, the handler appends --allowedTools to the claude invocation.
func TestOracleAsk_AgentTools_ForwardedAsAllowedTools(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
...
// TestOracleAsk_PerCallTools_WinsOverAgentTools verifies D5: when the effect
// sets `tools:` and the agent also declares Tools, the per-call list wins.
func TestOracleAsk_PerCallTools_WinsOverAgentTools(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	promptPath := filepath.Join(dir, "p.md")
	if err := os.WriteFile(promptPath, []byte("inspect"), 0o644); err != nil {
...
// TestOracleAsk_AgentDefaultCwd verifies that when the agent declares
// DefaultCwd and the effect omits working_dir, the agent cwd is used.
func TestOracleAsk_AgentDefaultCwd(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	promptPath := filepath.Join(dir, "p.md")
	...
	_, err := host.OracleAskHandler(ctx, map[string]any{
		"prompt_path": promptPath,
		"agent":       "cwd-agent",
...
// TestOracleAsk_PerCallCwd_WinsOverAgentCwd verifies that an explicit
// working_dir arg wins over agent.DefaultCwd.
func TestOracleAsk_PerCallCwd_WinsOverAgentCwd(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	promptPath := filepath.Join(dir, "p.md")
	if err := os.WriteFile(promptPath, []byte("hi"), 0o644); err != nil {
...
// After Phases 2/3/4/5/7, no oracle.* handler is a stub. The dedicated
// per-handler tests live in their own files (oracle_decide_test.go,
// oracle_ask_test.go, oracle_task_test.go, oracle_extract_test.go,
// oracle_converse_test.go). The two tests below cover the argument-validation
// shapes that distinguish a live handler from the old stub envelope.

// TestOracleTaskHandler_MissingAgent verifies that the Phase 4 task handler
// rejects calls with no agent: argument.
func TestOracleTaskHandler_MissingAgent(t *testing.T) {
	t.Parallel()
	res, err := host.OracleTaskHandler(context.Background(), nil)
	if err != nil {
...
// TestOracleExtractHandler_Phase5_Implemented verifies that the extract handler
// is live (Phase 5) and returns a schema-required error on nil args instead of
// the old "not yet implemented" stub response.
func TestOracleExtractHandler_Phase5_Implemented(t *testing.T) {
	t.Parallel()
	res, err := host.OracleExtractHandler(context.Background(), nil)
	if err != nil {
		t.Fatalf("unexpected Go error: %v", err)
	}
