...
// TestOracleExtract_SynonymsResolver_Hit verifies that a matching synonym entry
// is returned as submitted with resolved_by=synonyms.
func TestOracleExtract_SynonymsResolver_Hit(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
...
`)
	...
	res, err := host.OracleExtractHandler(ctx, map[string]any{
		"input":  "go north",
		"schema": schemaPath,
		"resolvers": []any{
...
// TestOracleExtract_SynonymsResolver_CaseInsensitive verifies that synonym
// matching is case-insensitive.
func TestOracleExtract_SynonymsResolver_CaseInsensitive(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	synonymPath := writeSynonymFile(t, dir, "s.yaml", `"Go North": {"direction": "north"}`)
	schemaPath := writeSchemaFile(t, dir, "schema.json")

	res, err := host.OracleExtractHandler(context.Background(), map[string]any{
		"input":  "go north",
		"schema": schemaPath,
		"resolvers": []any{
			map[string]any{"synonyms": synonymPath},
...
// TestOracleExtract_SynonymsMiss_FallsThrough verifies that a synonym miss
// falls through to the next resolver.
func TestOracleExtract_SynonymsMiss_FallsThrough(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	synonymPath := writeSynonymFile(t, dir, "s.yaml", `"go north": {"direction": "north"}`)
	schemaPath := writeSchemaFile(t, dir, "schema.json")
...
// TestOracleExtract_FirstMatchWins verifies that when multiple resolvers are
// declared, the first matching one wins (synonyms before LLM).
func TestOracleExtract_FirstMatchWins(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	synonymPath := writeSynonymFile(t, dir, "s.yaml", `"go north": {"direction": "north"}`)
	schemaPath := writeSchemaFile(t, dir, "schema.json")
...
// TestOracleExtract_LLMTier_CalledOnMiss verifies that the LLM tier is invoked
// when deterministic tiers don't match.
func TestOracleExtract_LLMTier_CalledOnMiss(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	synonymPath := writeSynonymFile(t, dir, "s.yaml", `"go north": {"direction": "north"}`)
	schemaPath := writeSchemaFile(t, dir, "schema.json")
...
// TestOracleExtract_LLMTier_Tools verifies that agent tools are forwarded via
// --allowedTools when the LLM tier runs.
func TestOracleExtract_LLMTier_Tools(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	schemaPath := writeSchemaFile(t, dir, "schema.json")
	promptPath := writePromptFile(t, dir, "p.md", "extract direction from input")
...
// TestOracleExtract_LLMTier_RejectsMutationTools verifies the extract LLM tier
...
func TestOracleExtract_LLMTier_RejectsMutationTools(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	schemaPath := writeSchemaFile(t, dir, "schema.json")
...
// TestOracleExtract_NoMatch_ReturnsNoMatchResult verifies the explicit no-match
// path: all tiers decline → resolved_by: no_match, submitted: nil.
func TestOracleExtract_NoMatch_ReturnsNoMatchResult(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	synonymPath := writeSynonymFile(t, dir, "s.yaml", `"go north": {"direction": "north"}`)
	schemaPath := writeSchemaFile(t, dir, "schema.json")
...
// TestOracleExtract_SchemaRequired verifies that omitting schema returns an
// error.
func TestOracleExtract_SchemaRequired(t *testing.T) {
	t.Parallel()
	res, err := host.OracleExtractHandler(context.Background(), map[string]any{
		"input": "go north",
	})
...
// TestOracleExtract_LLMTier_NonJSONFallsThrough verifies that when the LLM
// returns non-JSON output, it is treated as a no-match for that tier and falls
// through to the next resolver.
func TestOracleExtract_LLMTier_NonJSONFallsThrough(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	synonymPath := writeSynonymFile(t, dir, "s.yaml", `"go north": {"direction": "north"}`)
	schemaPath := writeSchemaFile(t, dir, "schema.json")
...
// TestOracleExtract_Validator_RejectsAndFallsThrough verifies that when a
...
func TestOracleExtract_Validator_RejectsAndFallsThrough(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
...
// TestOracleExtract_Streaming_EmitsResolverMatchedEvent verifies that after a
...
func TestOracleExtract_Streaming_EmitsResolverMatchedEvent(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
...
// TestOracleExtract_ParsedArgs_Resolvers verifies the resolver arg parsing for
// all three resolver types.
func TestOracleExtract_ParsedArgs_Resolvers(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	schemaPath := writeSchemaFile(t, dir, "schema.json")
	...
	res, err := host.OracleExtractHandler(context.Background(), args)
	if err != nil {
		t.Fatalf("unexpected Go error: %v", err)
	}
...
// TestOracleExtract_NoResolvers_NoMatch verifies that an invocation with no
// resolvers declared returns no_match.
func TestOracleExtract_NoResolvers_NoMatch(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	schemaPath := writeSchemaFile(t, dir, "schema.json")
...
// TestOracleExtract_LLMTier_ClaudeSessionIDPropagated verifies that
// claude_session_id is set in the result when the LLM tier matches.
func TestOracleExtract_LLMTier_ClaudeSessionIDPropagated(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	schemaPath := writeSchemaFile(t, dir, "schema.json")
	promptPath := writePromptFile(t, dir, "p.md", "extract")
...
// TestOracleExtract_LoadTimeSchemaMismatch verifies that a synonym payload that
// fails JSON schema validation causes the tier to fall through (runtime safety net).
func TestOracleExtract_LoadTimeSchemaMismatch(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
...
// TestOracleExtract_MultiPhrase_Synonyms verifies that comma-separated synonym
// phrases all resolve to the same payload.
func TestOracleExtract_MultiPhrase_Synonyms(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
...
// TestOracleExtract_JSONResult_IsParseable verifies that the synonym payload
// can be round-tripped through JSON encoding.
func TestOracleExtract_JSONResult_IsParseable(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
...
// TestOracleExtract_LLMTier_PerCallToolsAsStringSlice verifies that per-call
// tools passed as []string (not []any) are handled correctly (L9 type-safety).
func TestOracleExtract_LLMTier_PerCallToolsAsStringSlice(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	schemaPath := writeSchemaFile(t, dir, "schema.json")
	promptPath := writePromptFile(t, dir, "p.md", "extract")
...
// TestOracleExtract_LLMTier_DefaultPrompt_IncludesSchema verifies that when
// no explicit prompt is set, the default prompt includes the schema bytes so
// the LLM knows the required output shape (M5).
func TestOracleExtract_LLMTier_DefaultPrompt_IncludesSchema(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	...
	_, err := host.OracleExtractHandler(ctx, map[string]any{
		"input":  "go north",
		"schema": schemaPath,
		"resolvers": []any{
