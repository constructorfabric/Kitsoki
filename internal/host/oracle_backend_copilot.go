// Package host — the copilot oracleBackend.
//
// copilotBackend drives GitHub's `copilot` CLI as a drop-in alternative to
// `claude` for every oracle verb. The verb handlers are unaware of it: they
...
const CopilotBinEnv = "KITSOKI_ORACLE_COPILOT_BIN"
...
// "mcp__<server>__submit". The gated live smoke test (oracle_copilot_smoke)
