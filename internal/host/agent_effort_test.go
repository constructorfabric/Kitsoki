...
func TestAgentEffort_AgentForwarded(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
...
func TestAgentEffort_InlineWinsOverAgent(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	promptFile := filepath.Join(dir, "p.md")
	if err := os.WriteFile(promptFile, []byte("inspect"), 0o644); err != nil {
...
func TestAgentEffort_AbsentNoFlag(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	promptFile := filepath.Join(dir, "p.md")
	if err := os.WriteFile(promptFile, []byte("inspect"), 0o644); err != nil {
