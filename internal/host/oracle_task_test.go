...
// TestOracleTask_MissingAgent verifies that the handler rejects calls without
// an agent: arg.
func TestOracleTask_MissingAgent(t *testing.T) {
	t.Parallel()
	ctx := host.WithClaudeRunner(context.Background(), host.FakeTask(`{"ok":true}`))
	res, err := host.OracleTaskHandler(ctx, map[string]any{})
	if err != nil {
...
// TestOracleTask_UnknownAgent verifies that the handler rejects an unknown
// agent name.
func TestOracleTask_UnknownAgent(t *testing.T) {
	t.Parallel()
...
// TestOracleTask_MissingSchema verifies that the handler rejects calls without
// acceptance.schema.
func TestOracleTask_MissingSchema(t *testing.T) {
	t.Parallel()
	ctx := host.WithClaudeRunner(
		host.WithAgents(context.Background(), map[string]host.Agent{
...
// TestOracleTask_ModeA_ReplayFromDiff verifies the Mode A replay contract:
// given the initial hash + diff, the final state can be reconstructed.
func TestOracleTask_ModeA_ReplayFromDiff(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	if err := taskTestGitInit(dir); err != nil {
		t.Skip("git not available:", err)
...
// TestOracleStreamer_ReturnedSessionID verifies that the sessionID returned by
// OracleStreamer.Run (from system.init) is captured by the task handler for
...
func TestOracleStreamer_ReturnedSessionID(t *testing.T) {
	t.Parallel()
	...
	cr, sid, err := host.OracleStreamerRunExport(ctx, "stub://claude", []string{"-p"}, "", "")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	_ = cr
