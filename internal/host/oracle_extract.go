...
//     Streams tokens through OracleStreamer; honours agent.Tools (D5).
//
...
// ExtractValidatorDef is the optional validator: block on host.oracle.extract.
type ExtractValidatorDef struct {
	PostCmd     string
	PostCmdArgs map[string]any
}

// ExtractArgs is the parsed args for an OracleExtractHandler call.
...
func parseExtractArgs(args map[string]any) (ExtractArgs, string) {
	...
	if ea.SchemaPath == "" {
		return ea, "host.oracle.extract: schema argument is required"
	}
...
// OracleExtractHandler implements host.oracle.extract. It runs the tiered
...
func OracleExtractHandler(ctx context.Context, args map[string]any) (Result, error) {
...
func runExtract(ctx context.Context, ea ExtractArgs, bin string, rawArgs map[string]any) (Result, error) {
	if len(ea.Resolvers) == 0 {
		return Result{
			...
			Error: "host.oracle.extract: no resolvers declared and no match",
		}, nil
	}
		...
		for _, t := range effectiveTools(ctx, mergedArgs, agent) {
			if mutationTools[t] {
				return Result{Error: fmt.Sprintf(
					"host.oracle.extract: mutation tool %q not permitted in the LLM tier", t)}, nil
			}
		}
	}
	...
	return Result{
		...
		Error: "host.oracle.extract: no resolver matched",
	}, nil
}
...
func tryLLMResolver(ctx context.Context, ea ExtractArgs, lc *ExtractLLMConfig, bin string, rawArgs map[string]any) (any, string, bool, error) {
	...
	for attempt := 0; attempt < extractLLMMaxRetries; attempt++ {
		...
		cr, sessionID, runErr := OracleStreamer{
			Bin:        bin,
			CLIArgs:    fullCLIArgs,
			Stdin:      rendered,
