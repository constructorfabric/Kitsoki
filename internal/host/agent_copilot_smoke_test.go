...
//
//	KITSOKI_AGENT_LIVE=1 go test ./internal/host -run TestCopilotLiveSmoke -v
//
...
func TestCopilotLiveSmoke(t *testing.T) {
	if os.Getenv("KITSOKI_AGENT_LIVE") != "1" {
		t.Skip("set KITSOKI_AGENT_LIVE=1 to run the live copilot smoke test (real Copilot request)")
	}
