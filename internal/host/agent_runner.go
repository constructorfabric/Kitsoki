// Package host — internal helpers shared between the one-shot agent
// handlers (host.agent.ask, host.agent.ask_with_mcp). Both run the
...
type ClaudeRun struct {
	...
	// recordAgentUsage) so AgentReturned events surface per-invocation
...
// stdout line by line, emitting a "metamode.agent.event" slog record
...
func emitClassified(ctx context.Context, ce classifiedEvent) {
	...
	slog.InfoContext(ctx, "metamode.agent.event", attrs...)
}
...
// resolveAgentBin returns the path to the binary of the agent backend
...
func resolveAgentBin(ctx context.Context) (string, error) {
	return AgentBackendFromContext(ctx).ResolveBin(ctx)
}

// resolveClaudeBin returns the path to the claude binary, honoring
// AgentBinEnv and falling back to a PATH lookup. Returns
// ErrAgentUnavailable if neither is set.
//
...
func resolveClaudeBin(ctx context.Context) (string, error) {
	...
	if bin := os.Getenv(AgentBinEnv); bin != "" {
		return bin, nil
	}
