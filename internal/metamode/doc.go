...
//     [OracleCaller] seam — both interfaces, both injected. Tests pass
//     fakes; production passes the adapters built by
//     [NewChatStoreAdapter] and [NewOracleCallerAdapter].
//
...
//     returns [ErrChatBusy].
//  2. Append the user message FIRST, so a later oracle failure still
...
//     is used by host.runOracleAskWithMCPWithChat so transcripts stay
...
//	oracle.Ask -> reply + edits app.yaml
...
// oracle and chat store) lives in [ExampleController_Send].
//
// # Lifecycle
//
...
// host.ChatStore surface). [OracleCaller] wraps
// host.OracleAskWithMCPHandler, which imposes three constraints the
...
//     row records it), but host.OracleAskWithMCPHandler's non-chat path
...
// contract, and the oracle-verb mapping — is docs/stories/meta-mode.md.
