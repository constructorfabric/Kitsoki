...
// TestAgentCallRoundTrip verifies that KindAgentCall entries can be inserted
// into the journal and loaded back via LoadAgentCalls keyed by call_id.
//
...
func TestAgentCallRoundTrip(t *testing.T) {
	t.Parallel()
	...
	sid := app.SessionID("test-session-agent-001")
	turn := app.TurnNumber(2)
	callID := "call-abc-123"

	// Build a minimal AgentCallBody-shaped payload.
	body := map[string]any{
		"call_id":       callID,
		"verb":          "decide",
		"agent":         "my-agent",
	...
	entry := journal.Entry{
		...
		Turn:    turn,
		Seq:     5,
		Kind:    journal.KindAgentCall,
		Body:    bodyJSON,
	}
	require.NoError(t, w.Append(entry), "append agent call entry")
	...
	require.NoError(t, w.Append(entry2), "append second agent call entry")

	// Load agent calls and verify both entries are keyed by call_id.
	calls, err := journal.LoadAgentCalls(db, sid)
	require.NoError(t, err, "LoadAgentCalls must succeed")
	require.Len(t, calls, 2, "expect 2 agent call entries")
	...
	otherCalls, err := journal.LoadAgentCalls(db, otherSID)
	require.NoError(t, err)
	assert.Empty(t, otherCalls, "entries from a different session must not bleed through")
}

// TestKindAgentCall_IsTypedKind verifies that KindAgentCall is classified as
// a typed (semantic) kind, not a patch or checkpoint kind.
func TestKindAgentCall_IsTypedKind(t *testing.T) {
	t.Parallel()

	assert.True(t, journal.IsTypedKind(journal.KindAgentCall),
		"KindAgentCall must be a typed kind")
	assert.False(t, journal.IsPatchKind(journal.KindAgentCall),
		"KindAgentCall must not be a patch kind")
	assert.False(t, journal.IsCheckpointKind(journal.KindAgentCall),
		"KindAgentCall must not be a checkpoint kind")
}
