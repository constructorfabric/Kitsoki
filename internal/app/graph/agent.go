...
// AgentContract describes one host.agent.* invoke a room makes, with enough
// keying for the agent workbench (slice 3) to locate its cassette episodes.
type AgentContract struct {
	// Kind is the agent verb handler, e.g. "host.agent.decide".
...
// CassetteKey is the cassette-match tuple for an agent call, mirroring the
...
// AgentContracts walks a room's on_enter and intent-arc effects and returns
// one AgentContract per host.agent.* invoke. roomID is the top-level state id.
// It is a pure function over the app.
func AgentContracts(a app.App, roomID string) []AgentContract {
	id := roomOf(roomID)
	if id == "" {
		id = roomID
	}
