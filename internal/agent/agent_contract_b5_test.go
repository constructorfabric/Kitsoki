// Package agent — B-5 agent contract test suite.
//
// This file covers the "Testing the agent contract" sub-cases that were
// not already pinned by B-1 through B-4 tests.  Every test here is a hard CI
// gate.
//
// Sub-cases covered (see docs/architecture/agent-plugin.md for the full spec):
//
//	Schema validation:
//	  TestB5_Schema_RefFails                — $ref to sibling file fails (no filesystem loader)
//
//	Lifecycle and timeouts:
//	  TestB5_SubeventsBeforeCrash           — sub-events written before crash stay; AgentError closes call
...
//
// At the agent layer (validate.go) the compiler has no filesystem loader,
...
// and the agent surfaces the context error.
//
...
// TestB5_InProcess_HardTimeout pins the behaviour where an in-process agent
// blocks until ctx is done.  When ctx expires the agent returns an error
...
// passed to the agent is preserved in the AskResponse path and does not
// change across multiple Ask calls with the same request parameters.
//
// Note: DeriveCallID lives in internal/host, not internal/agent.  Here we
// test the agent-layer contract: CallID set on AskRequest passes through
// to the agent and is not mutated.
...
// TestB5_CallID_CollisionDetectionPin documents the behaviour when two agent
// calls would produce the same call_id.  Since DeriveCallID lives in
// internal/host, we pin the agent layer's contract: the agent itself does
...
//   - Agent transport DOES NOT reject duplicate CallIDs.
//   - EventSink DOES reject (or at minimum tolerate) duplicate call_ids on append.
//
// This test pins the agent-layer half of that contract: same CallID on two
// sequential asks both complete without error.
func TestB5_CallID_DuplicateAllowedByAgent(t *testing.T) {
	t.Parallel()
	...
	if calls != 2 {
		t.Errorf("expected 2 agent calls, got %d", calls)
	}
}
...
// test the observable effect at the agent package boundary: a plugin header
// whose value is a literal string containing "${" after expansion is neither
// re-expanded nor rejected.
//
// We simulate the single-pass rule directly using the agent AskRequest.WithArgs
// (which is opaque to the agent); no actual expansion happens here.  The real
// expansion is tested in internal/app/loader_agent_plugins_test.go.
// This test pins the documented behaviour:
//   - expandEnvVar("${AGENT_TOKEN}") with AGENT_TOKEN="val_${inner}" → "val_${inner}"
...
func TestB5_EnvVar_SinglePassExpansion(t *testing.T) {
	...
	// Instead we verify the documented contract via the agent plugin
	// path: that the agent transport receives headers/env verbatim (as
	// passed by the caller) without double-expansion.
	//
	// The in-process agent simply receives WithArgs as-is.  The test pins
	// that if the caller passes a header value containing "${" as a literal,
	// the agent receives it unchanged.
...
//
// The agent layer contract: the agent returns a Submission shaped by the
...
func TestB5_Secret_NotInSubmission(t *testing.T) {
	...
	fn := AskFunc(func(_ context.Context, req AskRequest) (AskResponse, error) {
		// Simulate a well-behaved agent: it uses the secret to authenticate
...
// appended to the JSONL are kept; AgentError closes the call."
//
// At the agent interface layer (not Dispatch), the agent contract says:
// AskResponse.SubEvents are returned alongside Submission.  When the agent
// returns an error, SubEvents is nil/empty.  The guarantee that sub-events
// land before the AgentError is a Dispatch-layer concern (covered by
// TestDispatch_SubEvents in agent_dispatch_test.go).
//
// This test pins the agent-layer contract: an agent MAY return both an error
...
// Dispatch (atomicity: AgentError replaces AgentReturned entirely).
// We test that the agent AskError carries the right kind for a crash path.
...
// responses and are handled identically at the agent layer.
