// agent.go holds the core plugin contract: the Agent interface, its
...
// plugin streaming an unbounded body; legitimate agent submissions are far
...
// Agent is the plugin contract for all agent transports.
...
type Agent interface {
	// Ask sends a rendered prompt to the agent and blocks until either a
	...
	// the context error so kitsoki can write AgentError and unblock the turn.
	...
	// Close releases any resources held by the agent (subprocess, HTTP client,
	// in-process state). Called once on session end; further Ask calls after
	// Close have undefined behaviour.
	Close() error
}

// AskRequest is the wire format sent to an agent on every Ask call.
...
type AskRequest struct {
	...
	// Verb is the opaque routing key identifying which agent host call
	...
	// source-color stripping have already been applied by the time the agent
	// receives this field.
	...
	// via ctx cancel. Plugins that overrun are surfaced as AgentError.
	Deadline time.Time `json:"deadline"`

	// CallID is the deterministic agent call identifier derived via
	// host.DeriveCallID. It pairs AgentCalled with AgentReturned / AgentError
	// in the JSONL trace. The caller (room dispatch) is responsible for deriving
	// and injecting this value.
	CallID string `json:"call_id"`
}

// AskResponse is the agent's reply to an Ask call.
// All fields must round-trip through JSON because subprocess and HTTP transports
// serialize the response.
type AskResponse struct {
	// Submission is the raw JSON produced by the agent. Kitsoki validates it
	...
	// the AgentReturned event payload under the "meta" key; it is never
	// interpreted by the state machine.
	Meta map[string]any `json:"meta,omitempty"`

	// SubEvents is an optional list of plugin-emitted events appended verbatim
	// to the JSONL between the AgentCalled and AgentReturned lines. Plugins
	...
	// sidecar and referenced from agent.call.complete by a single pointer.
	//
...
// Transcript is one agent call's backend-native execution detail, captured
...
// AskError is the typed error returned by Agent.Ask when the plugin fails
// before producing a usable Submission. Kitsoki writes AgentError to the trace
...
// closes the call with AgentError.
//
...
func (e *AskError) Error() string {
	if e == nil {
		return "agent: nil AskError"
	}
	if e.Detail != "" {
		return "agent: " + e.Kind + ": " + e.Detail
	}
	if e.Underlying != nil {
		return "agent: " + e.Kind + ": " + e.Underlying.Error()
	}
	return "agent: " + e.Kind
}
