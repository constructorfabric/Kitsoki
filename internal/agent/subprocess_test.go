...
//
// These tests use a compiled echo_agent binary (built at test time via go
...
// echoAgentBin is the path to the compiled echo_agent binary. Set once in
// TestMain (or via buildEchoAgent in each test that needs it).
var (
	echoAgentPath     string
	echoAgentPathOnce sync.Once
	echoAgentPathErr  error
)

// buildEchoAgent compiles the echo_agent helper binary and returns its path.
// The binary is compiled once per test run and cached in os.TempDir().
func buildEchoAgent(t *testing.T) string {
	t.Helper()
	echoAgentPathOnce.Do(func() {
		bin := filepath.Join(os.TempDir(), "kitsoki-echo-agent-test")
		if runtime.GOOS == "windows" {
			bin += ".exe"
		}
		cmd := exec.Command("go", "build", "-o", bin, "./testdata/echo_agent")
		cmd.Dir = mustGetwd()
		out, err := cmd.CombinedOutput()
		if err != nil {
			echoAgentPathErr = fmt.Errorf("build echo_agent: %w\n%s", err, string(out))
			return
		}
		echoAgentPath = bin
	})
	if echoAgentPathErr != nil {
		t.Skipf("echo_agent build failed (skip on CI without Go toolchain): %v", echoAgentPathErr)
	}
	return echoAgentPath
}
...
// TestSubprocessHappyPath verifies that the subprocess agent sends a request
...
// before producing any output, the agent returns *AskError{Kind: "plugin_crash"}.
...
// writes a partial frame and exits, the agent returns an error with partial
...
func TestSubprocessRespawnAfterCrash(t *testing.T) {
	...
	// The agent proc is nil after crash. Now clear the env and update the command
	// to be a working binary. The easiest way: create a new SubprocessAgent with
