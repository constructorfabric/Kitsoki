...
// NewMCPHTTP creates an MCPHTTPOracle. endpoint is the MCP server URL
...
func NewMCPHTTP(endpoint, tool string, headers map[string]string) *MCPHTTPOracle {
	if tool == "" {
		tool = "ask"
	}
	return &MCPHTTPOracle{
		endpoint: endpoint,
		tool:     tool,
		headers:  headers,
		client: &http.Client{
...
func (o *MCPHTTPOracle) Ask(ctx context.Context, req AskRequest) (AskResponse, error) {
	...
	if err != nil {
		return AskResponse{}, &AskError{
			...
			Underlying: err,
			Detail:     fmt.Sprintf("mcp_http oracle: marshal AskRequest: %v", err),
		}
	}
	...
	if err != nil {
		return AskResponse{}, &AskError{
			Kind:       "transport_error",
			Underlying: err,
			Detail:     fmt.Sprintf("mcp_http oracle: marshal tool call params: %v", err),
		}
	}
	...
	if err != nil {
		return AskResponse{}, &AskError{
			Kind:       "transport_error",
			Underlying: err,
			Detail:     fmt.Sprintf("mcp_http oracle: marshal rpc request: %v", err),
		}
	}
	...
	if err != nil {
		return AskResponse{}, &AskError{
			Kind:       "transport_error",
			Underlying: err,
			Detail:     fmt.Sprintf("mcp_http oracle: build http request: %v", err),
		}
	}
	...
	if err != nil {
		...
		if callCtx.Err() == context.DeadlineExceeded || ctx.Err() == context.DeadlineExceeded {
			return AskResponse{}, &AskError{
				...
				Underlying: err,
				Detail:     fmt.Sprintf("mcp_http oracle: request timed out: %v", err),
			}
		}
		if callCtx.Err() == context.Canceled || ctx.Err() == context.Canceled {
			return AskResponse{}, &AskError{
				Kind:       "deadline_exceeded",
				Underlying: err,
				Detail:     fmt.Sprintf("mcp_http oracle: request cancelled: %v", err),
			}
		}
		return AskResponse{}, &AskError{
			Kind:       "transport_error",
			Underlying: err,
			Detail:     fmt.Sprintf("mcp_http oracle: http do: %v", err),
		}
	}
	...
	if err != nil {
		return AskResponse{}, &AskError{
			Kind:       "transport_error",
			Underlying: err,
			Detail:     fmt.Sprintf("mcp_http oracle: read response body: %v", err),
		}
	}
	...
	if rpcResp.Error != nil {
		return AskResponse{}, &AskError{
			Kind:   "transport_error",
			Detail: fmt.Sprintf("mcp_http oracle: rpc error %d: %s", rpcResp.Error.Code, rpcResp.Error.Message),
		}
	}
	...
	if rpcResp.Result == nil {
		return AskResponse{}, &AskError{
			Kind:   "transport_error",
			Detail: "mcp_http oracle: response has neither result nor error",
		}
	}
	...
	if askRespText == "" {
		return AskResponse{}, &AskError{
			Kind:   "transport_error",
			Detail: "mcp_http oracle: response content has no text item",
		}
	}
...
func (o *MCPHTTPOracle) Close() error {
