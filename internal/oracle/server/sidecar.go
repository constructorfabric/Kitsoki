...
const (
	...
	// SIGTERM before escalating to SIGKILL. It mirrors oracle's
	// SubprocessTerminateTimeout; kept local to avoid an import cycle
	// (oracle imports this package for the managed sidecar).
	terminateTimeout = 2 * time.Second
)

// Process is the subset of a spawned OS process the sidecar needs: signal it and
// wait for exit. It mirrors the os/exec surface SubprocessOracle's terminateProc
...
// on first use, mirroring the oracle transports' eager-config / lazy-work split.
...
// EnsureRunning resolves the base URL the oracle should POST to. In endpoint mode
...
// healthTimeout elapses. It gates EnsureRunning so the first oracle POST never
...
// escalates to SIGKILL — copying SubprocessOracle's terminateProc so session
