...
// It is what the echo_oracle binary and the HTTP handler both return in the
// "echo" shape.
...
// same AskRequest.
//
// Reference oracle: an in-process oracle that produces the same echo shape as
// echo_oracle (the subprocess test binary) and the HTTP handler below.
func TestConformance_InProcessVsSubprocessVsHTTP(t *testing.T) {
	// Build echo oracle binary (reuses the cached binary from subprocess tests).
	echobin := buildEchoOracle(t)

	req := conformanceRequest()

	// ── 1. In-process oracle ──────────────────────────────────────────────────
	inprocOracle := New(AskFunc(func(_ context.Context, r AskRequest) (AskResponse, error) {
		head := r.PromptText
		if len(head) > 50 {
			head = head[:50]
		}
	...
	// ── 2. Subprocess oracle ──────────────────────────────────────────────────
	subprocOracle := NewSubprocess(echobin, nil, nil)
	...
	httpOracle, httpSrv := buildEchoHTTPServer(t)
...
// oracle behaviour as echo_oracle (subprocess). Returns the MCPHTTPOracle and
// the test server (caller must defer Close on both).
func buildEchoHTTPServer(t *testing.T) (*MCPHTTPOracle, *httptest.Server) {
	t.Helper()
...
// plugins into a Registry, resolving oracle.subprocess and oracle.http
// produces oracles that work correctly.
