// Runnable godoc examples for the oracle plugin contract. Each Example
// function's // Output: block is checked by
// `go test -run "^Example" ./internal/oracle/...`.
package oracle_test
...
// AskFunc as an Oracle, ask it under a schema, and validate the submission —
// the same happy-path trace shown in the package doc.
func ExampleNew() {
	// A stub oracle that always submits a fixed decision.
	o := oracle.New(func(_ context.Context, req oracle.AskRequest) (oracle.AskResponse, error) {
	...
	if verr := oracle.ValidateSubmission(schema, resp.Submission); verr != nil {
		panic(verr)
	}
...
// submission missing a required property yields an *oracle.AskError with
// Kind "schema_invalid".
func ExampleValidateSubmission_schemaInvalid() {
	...
	err := oracle.ValidateSubmission(schema, submission)
...
// and alias resolution, including the default-oracle fallback for an unknown
// alias.
func ExampleBuildRegistry() {
	// A stub in-process oracle injected under the default alias.
	stub := oracle.New(func(_ context.Context, _ oracle.AskRequest) (oracle.AskResponse, error) {
	...
	reg, err := oracle.BuildRegistry(map[string]*oracle.PluginDecl{}, nil)
	if err != nil {
		panic(err)
	}
	reg.Register(oracle.DefaultOracleName, stub)
	defer reg.Close()

	// An explicit alias resolves to itself; an unknown alias falls back to the
	// default rather than erroring.
	def, errDefault := reg.Resolve(oracle.DefaultOracleName)
	fallback, errUnknown := reg.Resolve("oracle.unknown")

	fmt.Println("default name:", oracle.DefaultOracleName)
	fmt.Println("resolved default:", def != nil && errDefault == nil)
	fmt.Println("unknown falls back (no error):", fallback != nil && errUnknown == nil)
	// Output:
	// default name: oracle.claude
