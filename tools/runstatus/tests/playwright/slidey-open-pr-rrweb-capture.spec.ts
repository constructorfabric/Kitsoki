/**
 * slidey-open-pr-rrweb-capture.spec.ts — rrweb capture spec (the slidey bug-fix tour).
 *
 ...
 * DOM-mutation stream of the slidey-open-pr tour for embedding as a NATIVE rrweb
 ...
 *
 *   - It drives the SLIDEY_OPEN_PR_TOUR_STEPS manifest against the slidey-open-pr
 *     story instance (stories/slidey-open-pr) + its no-LLM flow
 ...
 *     runner.go), which is what `kitsoki tour --feature slidey-open-pr` runs. The
 *     manifest is self-driving: it advances the deterministic flow itself
 *     (bf__start → bf__accept × N), so NO patch_world is needed — the
 *     slidey-open-pr instance bakes ticket_id/workdir/etc. into its world DEFAULTS
 *     (and bf_autostart_attempted=true parks bf.idle for the explicit start).
 *
 * Artifacts (all under .artifacts/rrweb-eval/slidey-open-pr/):
 *   - slidey-open-pr.rrweb.json          ← the captured rrweb event stream
 *   - slidey-open-pr.rrweb.capture.json  ← viewport sidecar (width/height/dsf)
 *   - slidey-open-pr-baseline.mp4        ← the live screen-recording (the BASELINE)
 *   - baseline-frames/NN-*.png         ← per-step baseline screenshots
 *
 * Run at watch-speed:
 *   pnpm exec playwright test slidey-open-pr-rrweb-capture --project=chromium
 */
...
import { SLIDEY_OPEN_PR_TOUR_STEPS, type TourStep } from "../../src/tour/generated/slidey-open-pr.js";

const CHAPTER_SOURCE = "features/slidey-open-pr.yaml";

// Distinct port so this capture can run alongside other specs without racing.
const ADDR = "127.0.0.1:7757";
// pr-refinement is the standalone PR-refinement pipeline; the slidey-open-pr
// feature points its demo.story at it. The slidey_open_pr flow carries its own
// inline host_handlers (no separate cassette).
const STORY_DIR = path.join(repoRoot, "stories", "pr-refinement");
const FLOW = path.join(STORY_DIR, "flows", "slidey_open_pr.yaml");

const ARTIFACT_DIR = path.join(repoRoot, ".artifacts", "rrweb-eval", "slidey-open-pr");
const BASELINE_FRAMES_DIR = path.join(ARTIFACT_DIR, "baseline-frames");
const VIDEO_DIR = path.join(ARTIFACT_DIR, "_baseline-video");
const EVENTS_JSON = path.join(ARTIFACT_DIR, "slidey-open-pr.rrweb.json");
...
test("slidey bug-fix rrweb capture (baseline + event stream)", async () => {
  ...
  try {
    ...
    }, JSON.stringify(SLIDEY_OPEN_PR_TOUR_STEPS));
    await expect(page.getByTestId("tour-overlay")).toBeVisible({ timeout: 8000 });

    // ── 2. Walk the SLIDEY_OPEN_PR_TOUR_STEPS (port of walkSteps) ───────────────
    let pngIdx = 0;
    for (const step of SLIDEY_OPEN_PR_TOUR_STEPS) {
    ...
    await context.close();
    const mp4 = await saveVideoAsMp4(video, ARTIFACT_DIR, "slidey-open-pr-baseline");
    writeChapters(mp4, chapters.list());
    await browser.close();
  }

  const pngs = fs.readdirSync(BASELINE_FRAMES_DIR).filter((f) => f.endsWith(".png"));
  console.log(`[slidey-open-pr-rrweb-capture] baseline frames (${pngs.length}) in ${BASELINE_FRAMES_DIR}`);
  console.log(`[slidey-open-pr-rrweb-capture] events → ${EVENTS_JSON}`);
});
