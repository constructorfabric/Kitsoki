/**
 * gears-bugfix-rrweb-capture.spec.ts — rrweb capture spec (the gears bug-fix tour).
 *
 ...
 * DOM-mutation stream of the gears-bugfix tour for embedding as a NATIVE rrweb
 ...
 *
 *   - It drives the GEARS_BUGFIX_TOUR_STEPS manifest against the gears-bugfix
 *     story instance (stories/gears-bugfix) + its no-LLM flow
 ...
 *     runner.go), which is what `kitsoki tour --feature gears-bugfix` runs. The
 *     manifest is self-driving: it advances the deterministic flow itself
 *     (bf__start → bf__accept × N), so NO patch_world is needed — the
 *     gears-bugfix instance bakes ticket_id/workdir/etc. into its world DEFAULTS
 *     (and bf_autostart_attempted=true parks bf.idle for the explicit start).
 *
 * Artifacts (all under .artifacts/rrweb-eval/gears-bugfix/):
 *   - gears-bugfix.rrweb.json          ← the captured rrweb event stream
 *   - gears-bugfix.rrweb.capture.json  ← viewport sidecar (width/height/dsf)
 *   - gears-bugfix-baseline.mp4        ← the live screen-recording (the BASELINE)
 *   - baseline-frames/NN-*.png         ← per-step baseline screenshots
 *
 * Run at watch-speed:
 *   pnpm exec playwright test gears-bugfix-rrweb-capture --project=chromium
 */
...
test("gears bug-fix rrweb capture (baseline + event stream)", async () => {
  ...
  try {
    ...
    }, JSON.stringify(GEARS_BUGFIX_TOUR_STEPS));
    await expect(page.getByTestId("tour-overlay")).toBeVisible({ timeout: 8000 });

    // ── 2. Walk the GEARS_BUGFIX_TOUR_STEPS (port of walkSteps) ───────────────
    let pngIdx = 0;
    for (const step of GEARS_BUGFIX_TOUR_STEPS) {
    ...
    await context.close();
    const mp4 = await saveVideoAsMp4(video, ARTIFACT_DIR, "gears-bugfix-baseline");
    writeChapters(mp4, chapters.list());
    await browser.close();
  }

  const pngs = fs.readdirSync(BASELINE_FRAMES_DIR).filter((f) => f.endsWith(".png"));
  console.log(`[gears-bugfix-rrweb-capture] baseline frames (${pngs.length}) in ${BASELINE_FRAMES_DIR}`);
  console.log(`[gears-bugfix-rrweb-capture] events → ${EVENTS_JSON}`);
});
