...
import AgentWorkbench from "../../src/components/editor/AgentWorkbench.vue";
...
function fakeSource(over: Record<string, unknown> = {}) {
  return {
    editorAgents: vi.fn().mockResolvedValue({
      contracts: [
        {
          kind: "host.agent.decide",
          prompt_path: "prompts/p.txt",
          output_schema: "schemas/s.json",
          cassette_key: { handler: "host.agent.decide", phase: "clarifying", call: "pick" },
          effect_index: 0,
        },
      ],
      cassette_globs: [],
...
describe("AgentWorkbench", () => {
  it("renders a card per contract with verb + prompt + schema", async () => {
    const w = mount(AgentWorkbench, {
      props: { source: fakeSource(), storyPath: "/s/app.yaml", roomId: "clarifying" },
    });
    await flushPromises();
    const card = w.find('[data-testid="editor-agent-card"]');
    expect(card.exists()).toBe(true);
    expect(card.text()).toContain("host.agent.decide");
  ...
  it("replays a cassette and emits the world snapshot", async () => {
    ...
    await flushPromises();
    await w.find('[data-testid="editor-agent-replay-cassette"]').trigger("click");
    await flushPromises();
    expect(w.find('[data-testid="editor-agent-output"]').text()).toContain("yes");
  ...
  it("disables the live replay button", async () => {
    ...
    await flushPromises();
    const live = w.find('[data-testid="editor-agent-replay-live"]');
