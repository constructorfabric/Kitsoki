# Agent Event attrs Shape

This document defines the canonical `attrs` shape for `agent.*` trace events.
...
```
agent.call.start     lightweight; emitted when the LLM call begins
agent.call.complete  full; emitted when the LLM call returns
```

The verb lives in `attrs.verb`, **not** in the event name. This is the single
canonical agent trace shape the engine emits; the older `agent.<verb>.start`
naming (which the engine never produced) has been retired. `attrs.verb` is one
of: `decide`, `extract`, `ask`, `task`, `converse`.

### agent.call.start attrs
...
| prompt_file? | string | Relative path (from the trace dir) to the on-disk prompt sidecar at `agent-prompts/<call_id>.txt`. Present when the rendered prompt is large (>1KB) and a prompts dir is configured; omitted otherwise. |
...
Verb-specific input provided to the agent operator. Only the keys relevant
...
`{trace_dir}/agent-prompts/{call_id}-response.json` and the `.complete`
...
### decide (agent.call.complete, verb="decide")
...
### extract (agent.call.complete, verb="extract")
...
### ask (agent.call.complete, verb="ask")
...
### task (agent.call.complete, verb="task")
...
### converse (agent.call.complete, verb="converse")
