#!/usr/bin/env python3
"""
Generate agent-rich.snapshot.json — a synthetic but realistic-looking
snapshot that exercises all five agent verbs (decide, extract, ask, task,
converse) with the full rich attrs shape described in AGENT_ATTRS.md.
...
Usage:
    python3 agent-rich.gen.py > agent-rich.snapshot.json
or via Makefile:
    make -C tools/runstatus/fixtures agent-rich
"""
...
# (prompt templates / transition tables) that are not needed for agent-attrs
...
def turn1_decide() -> None:
    ...
    # agent.call.start (verb=decide) — lightweight early-visibility event
    emit("DEBUG", "agent.call.start", turn, sp,
         verb="decide",
    ...
    # agent.call.complete (verb=decide) — full rich event
    emit("DEBUG", "agent.call.complete", turn, sp, step=0.01,
         verb="decide",
         agent="strategy_router",
         model=MODEL,
         duration_ms=840,
...
def turn2_extract() -> None:
    ...
    # agent.call.complete (verb=extract)
    emit("DEBUG", "agent.call.complete", turn, sp, step=0.01,
         verb="extract",
         agent="intake_extractor",
         model=MODEL,
...
def turn4_task() -> None:
    ...
    # agent.call.complete (verb=task) — rich event with tool_calls + files_changed
    emit("DEBUG", "agent.call.complete", turn, sp, step=0.01,
         verb="task",
         agent="reproducing_specialist",
         model=MODEL,
...
def turn5_converse() -> None:
    ...
    # agent.call.start (verb=converse)
    emit("DEBUG", "agent.call.start", turn, sp,
         verb="converse",
    ...
    # agent.call.complete (verb=converse)
    emit("DEBUG", "agent.call.complete", turn, sp, step=0.01,
         verb="converse",
         agent="clarification_agent",
         model=MODEL,
