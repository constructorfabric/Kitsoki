#!/usr/bin/env python3
"""
Generate agent-with-separate-prompts.snapshot.json — similar to agent-rich
...
SESSION = "sess-agent-separate-prompts"
APP_ID = "bugfix"
...
def turn1_decide() -> None:
    ...
    emit("DEBUG", "agent.call.complete", turn, sp, step=0.01,
         ...
         response_tokens=18,
         cost_usd=0.0009,
         system_prompt_file="agent-prompts/decide-001-system.txt",
         prompt_file="agent-prompts/decide-001.txt",
         input={
             "choices": [
...
def turn2_extract() -> None:
    ...
    emit("DEBUG", "agent.call.complete", turn, sp, step=0.01,
         ...
         response_tokens=42,
         cost_usd=0.00071,
         system_prompt_file="agent-prompts/extract-001-system.txt",
         prompt_file="agent-prompts/extract-001.txt",
