...
// Every agent call (complete OR error) carrying a transcript_ref becomes a row,
...
const groups = computed<Group[]>(() => {
  ...
  for (const ev of props.events) {
    const a = ev.attrs ?? {};
    if (ev.msg !== "agent.call.complete" && ev.msg !== "agent.call.error") continue;
    ...
    const row: CallRow = {
      ...
      isError: ev.msg === "agent.call.error",
      turn,
      state: ev.state_path ?? "",
