...
/**
 * AgentWorkbench — for the selected room, renders one card per host.agent.*
 * contract (runstatus.editor.agents). Each card shows the verb / prompt /
 ...
 * mirrors AgentDetail's intent: show the recorded agent output for review).
 */
...
async function load(): Promise<void> {
  ...
  replayError.value = {};
  try {
    const res = await props.source.editorAgents(props.storyPath, props.roomId);
...
<template>
  <section class="workbench" data-testid="editor-agent-workbench">
    <h3 class="workbench__title">Agent workbench</h3>
    <div v-if="loading" class="workbench__status">Loading agent contracts…</div>
    ...
    <p v-else-if="contracts.length === 0" class="workbench__empty">
      This room makes no agent calls.
    </p>
    <div v-else class="workbench__cards">
      ...
      >
        ...
        <div
          ...
          data-testid="editor-agent-output"
        >
