#!/usr/bin/env bash
# integrate_room_hardening.sh — REAL regression guard for the integrate-room
# hardening in stories/git-ops/rooms/integrate.yaml (mirrors the delivery-tail
# guard). The flow fixtures MOCK integrate_exec, so they prove ROUTING but
# cannot exercise the bash. This script guards the bash mechanisms directly:
#
#   #8  gate-string collision — STRUCTURAL: the build check must be passed as a
#       POSITIONAL parameter to `bash -c`, never pasted into a shell-parsed
#       assignment (`BUILD_CMD="{{ world.build_check_cmd }}"`). FUNCTIONAL: the
...
#       a bare `set -e` abort.
#
# Run from repo root: bash stories/git-ops/tests/integrate_room_hardening.sh
...
if grep -Eq 'BUILD_CMD="\{\{[[:space:]]*world\.build_check_cmd' "$ROOM"; then
  note "FAIL: gate is interpolated into a shell-parsed assignment (the bug)."
  fail=1
else
  note "ok: no BUILD_CMD=\"{{ world.build_check_cmd }}\" assignment."
fi
...
if grep -q '"{{ world.build_check_cmd }}"' "$ROOM"; then
