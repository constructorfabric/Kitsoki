...
For Capsule CI operation or failure investigation, use
`.agents/skills/capsule-ci-operations/SKILL.md`. Run the no-spend
`kitsoki capsule ci doctor` preflight before a live or remote job, diagnose a
persisted run with `kitsoki capsule ci diagnose --job <id>` or `--latest`
before retrying it, and preserve both controller and worker evidence for an
open executor span. Treat `kitsoki capsule cleanup plan|apply` as the supported
disk-hygiene path; do not manually delete managed workspaces, run evidence, or
caches. Automated gates remain flow/cassette/fake-executor based and never
enable a live agent profile.
