// agent_serve_test.go — tests for `kitsoki agent-serve` (Phase 6).
//
...
// startTestAgentServer starts a runAgentServe in a goroutine and returns
// the socket path. The server stops when ctx is cancelled. socketPath is
// returned so callers can dial it.
func startTestAgentServer(t *testing.T, ctx context.Context) string {
	t.Helper()
	sockPath := filepath.Join(shortSocketDir(t), "agent-test.sock")
	...
	go func() {
		...
		_ = runAgentServe(ctx, sockPath, logOut)
	}()
	select {
	case <-ready:
	case <-time.After(3 * time.Second):
		t.Fatal("agent-serve did not start within 3 seconds")
	}
	return sockPath
}
...
func sendRPC(t *testing.T, sockPath string, req rpcRequest) rpcResponse {
	...
	for scanner.Scan() {
		raw := scanner.Bytes()
		// Peek at the "method" field: notification frames have method="agent.event"
...
func TestAgentServe_StartsAndAcceptsConn(t *testing.T) {
	t.Parallel()
	...
	conn, err := net.Dial("unix", sockPath)
	if err != nil {
		t.Fatalf("could not connect to agent-serve: %v", err)
	}
	conn.Close()
}
...
func TestAgentServe_UnknownMethod(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
func TestAgentServe_Extract_DomainErrorForMissingSchema(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
func TestAgentServe_Decide_DomainErrorForMissingSchema(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
func TestAgentServe_Ask_DomainErrorForMissingPrompt(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
func TestAgentServe_Task_DomainErrorForMissingAgent(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
func TestAgentServe_Converse_DomainErrorForMissingQuestion(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
func TestAgentServe_ParseError(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
// ── agentRPCCall via socket ───────────────────────────────────────────────

func TestAgentRPCCall_DomainErrorSurfaced(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	err := agentRPCCall(ctx, &sb, sockPath, "agent.decide", map[string]any{})
	// Decide requires schema: — the error should propagate back through agentRPCCall.
	if err == nil {
		t.Fatal("expected error back from agentRPCCall for missing schema")
	}
}

func TestAgentRPCCall_UnknownMethod(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	var sb strings.Builder
	err := agentRPCCall(ctx, &sb, sockPath, "agent.unknown", map[string]any{})
	if err == nil {
		t.Fatal("expected error for unknown method via agentRPCCall")
	}
...
// TestAgentServe_StreamingNotifications verifies the §5.2 streaming protocol:
...
func TestAgentServe_StreamingNotifications(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	// Use agent.task with missing agent — this will error fast (domain error),
	...
	// agent.converse with a FakeConverse stub that is pre-installed in the
	// server context. Since we can't install a per-test runner in the server,
	// we use agent.extract with a missing schema (fast domain error, no
	// notifications emitted) and verify the response shape is correct.
	// For the actual streaming assertion, we test dispatchAgentRPC directly.

	// Test the notify callback path: call dispatchAgentRPC with a notify func
	...
	// Write a prompt file so agent.ask can read it.
	...
	finalResp, dispatchErr = dispatchAgentRPC(handlerCtx, "agent.ask", map[string]any{
	...
	// 3 assistant events + 1 result event → 4 agent.event notifications.
...
// TestAgentServe_ConcurrentClientsOwnSessionIDs verifies C3: two concurrent
// agent-serve RPC calls with different parent_session_ids do not clobber each
// other's session ID. Each handler receives its own session ID via context.
func TestAgentServe_ConcurrentClientsOwnSessionIDs(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
// TestRunAgentServe_RejectsIfAlreadyListening verifies that a second
// agent-serve at the same path fails with "already running".
func TestRunAgentServe_RejectsIfAlreadyListening(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	err := runAgentServe(context.Background(), sockPath, &strings.Builder{})
	if err == nil {
...
// TestDelegateToSocket_AutoFallback verifies M11: when KITSOKI_AGENT_SOCK is
...
func TestDelegateToSocket_AutoFallback(t *testing.T) {
	...
	err := delegateToSocket(ctx, &sb, noSock, "agent.decide", map[string]any{})
...
// TestAgentServe_WriteDeadline_SlowClientUnblocks verifies N4: when a client
...
func TestAgentServe_WriteDeadline_SlowClientUnblocks(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
// TestAgentServe_CtxCancelClosesConn verifies N4: cancelling the server context
// closes in-flight connections so blocked enc.Encode calls return promptly.
func TestAgentServe_CtxCancelClosesConn(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithCancel(context.Background())
...
// TestAgentServe_PerCallTimeout_OverrideViaParams verifies N4: a
...
func TestAgentServe_PerCallTimeout_DefaultsApplied(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
