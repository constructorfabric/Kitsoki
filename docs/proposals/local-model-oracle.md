...
mapping, and a live A/B against `oracle.claude` inside a real story.
**Kind:**   runtime
**Epic:**   — standalone

## What shipped

The `builtin.local_llm` oracle is implemented, opt-in, and additive;
`oracle.claude` stays the default and every existing story/cassette is
untouched. The settled design and behaviour now live in the narrative docs:

- **Backend, grammar, fallback, lifecycle, acquisition:**
  [`docs/architecture/oracle-plugin.md` §9 "Local model backend"](../architecture/oracle-plugin.md).
...
- `internal/oracle/local_llm.go` — the OpenAI `/v1/chat/completions` `Oracle`
  ...
  guarantee.
- `internal/oracle/grammar_subset.go` + leaf package `internal/oracle/grammar`
  (`SubsetOK`) — the supported-subset gate, reused at load time.
- `internal/oracle/server/` — the managed sidecar: `Fetcher`/`Spawner` seams,
  ...
  (`internal/oracle/local_llm_e2e_test.go`, gated behind `KITSOKI_LLM_E2E=1`).
  An opt-in regression test asserts a managed oracle fetches nothing at
...
- Validation-reject fallback to `oracle.claude` (one `call_id`, recorded via
  the new `substitution` field on `OracleReturned`/`OracleError`, gated to
...
- `app.RoutingConfig.ExtractLLMOnNoMatch` (default off) + the `TrySemantic`
  ...
  `tools/oracle-fetch` (same `Fetcher`); `.gitignore` + `.gitkeep`'d cache dir.
...
- [ ] 3.2 Live A/B: point one real story's decide gate (e.g. the pr-refinement
          ...
          output correctly trips ValidateSubmission → the oracle.claude fallback.
...
- **Replacing `claude` for the hard verbs.** `ask`/`task`/`converse` and any
  genuinely reasoning-heavy `decide` stay on `oracle.claude`.
