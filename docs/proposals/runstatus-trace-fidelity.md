...
parallel columns, with prompt + response in every oracle row). The
canonical trace events (`oracle.call.*` with verb in attrs, `machine.say`,
...
(`artifact.spec.ts` ×5, `oracle-prompts.spec.ts` ×1) still assert the
...
- **Every LLM call is rendered twice.** Each oracle call appears as a separate
  `oracle.call.start` *and* `oracle.call.complete` row in both columns instead
  of one merged row — duplication in the same column, the exact thing the two
  columns are supposed to avoid.
- **The rich oracle detail never renders.** Expanding an oracle row shows a
...
- **Every phase header is duplicated.** The right column visits
  ...
  each phase appears twice, non-adjacent, with its oracle call severed from its
  host/world work.
...
- **3 tests are already red.** `tools/runstatus/tests/playwright/bugfix.spec.ts`
  fails its three oracle-detail assertions today (`6 passed, 3 failed`).

The root is a **split trace contract**: the real orchestrator emits oracle
events as `oracle.call.start/complete` with the verb in `attrs.verb`
(`internal/store/event.go:109,114`), but the runstatus consumer — and the five
hand-authored fixtures it was built against — assume `oracle.<verb>.start`
...
One sentence: **`oracle.call.*` (verb-in-attrs) becomes the single canonical
oracle trace shape; the trace stops conflating narration with world mutation
...
- **Producer (perfect trace).** Stamp oracle on-enter events with the real
  foreground turn (deleting the load-time `repairOracleTurns` hack); split
  ...
  flow-driven traces match live sessions; retire the dead `oracle.ask.*`
  naming and migrate the five synthetic fixtures to `oracle.call.*`.
- **Consumer (faithful representation).** Match `oracle.call.start/complete`
  and read the verb from `attrs.verb` (restores merge, suppression, and
  `OracleDetail`); group the right column by **phase** so it stays parallel to
...
- **Producers:**
  - `internal/host/oracle_dispatch.go` (`appendOracleCalledEventWithEpisode` ~296, `appendOracleReturnedEvent` ~360) — turn stamping + always-present prompt ref.
  ...
  - `internal/store/event.go:27` — retire dead `LLMCalled = "oracle.ask.start"`.
- **Consumers:** `tools/runstatus/src/components/{TraceTimeline,StateDiagram,EventDetail}.vue`, `src/components/oracle/*`, `src/data/snapshot-source.ts` (delete `repairOracleTurns`), `tools/runstatus/scripts/build-artifact.mjs` (bundle sidecars).
- **Format:** new `machine.say` event kind; oracle events gain a guaranteed prompt reference; no field removed.
- **Backward compat:** no *real* recorded trace uses `oracle.<verb>.*` — the engine never emitted it. Only the five synthetic fixtures do; they are migrated, not supported in perpetuity. `repairOracleTurns` deletion is safe once oracle turns are stamped correctly upstream.
- **Docs on ship:** `docs/tracing/trace-format.md` (oracle + say + intent events), `docs/tracing/turn.md`.

## Event / format model

Canonical oracle events (already what the engine emits — this proposal makes it
the *only* shape and fixes the consumer to it):

```jsonc
// oracle.call.start  — verb lives in attrs.verb, not the msg
{ "msg": "oracle.call.start", "turn": 2, "state_path": "proposing",
  "attrs": { "verb": "decide", "agent": "proposer", "model": "claude-sonnet-4-6",
             "call_id": "2d8e4fbb0a78646d", "prompt_file": "oracle-prompts/8bf4401a34847271.txt" } }
// oracle.call.complete
{ "msg": "oracle.call.complete", "turn": 2, "state_path": "proposing",
...
|---|---|---|
| `oracle.call.start` | on-enter oracle dispatch | stamp `turn` = foreground turn (not 0); always carry a prompt ref (inline `prompt` if small, else `prompt_file`) |
| `oracle.call.complete` | oracle returns | `turn` matches its start; `verb` + `response` + `duration_ms` |
...
- **Turn stamping must keep replay byte-identical.** The oracle events currently
...
consumer's `repairOracleTurns` tries to patch the turn back but computes
`nearestTurn(time ≤ oracle_time)`, which resolves to the **previous** turn for
...
spans two turns: *entering* a phase (on-enter oracle + artifact bind) happens in
...
**Consumer — the oracle match.** Replace the verb-enumerated regexes
(`TraceTimeline.vue:212-213`, `EventDetail.vue` ~174, `StateDiagram.vue` ~199)
with a match on `oracle.call.start` / `oracle.call.complete`, reading
`attrs.verb` for routing to `oracle/DecideDetail.vue`, `TaskDetail.vue`, etc.
This single change restores: start+complete merge into one row (no same-column
dup), the already-correct `host.oracle.*` suppression
(`TraceTimeline.vue:410,480`), and `OracleDetail` rendering with verb badge and
prompt/response panes.

## Backward compatibility

- **Synthetic fixtures** (`completed`, `edge-cases`, `in-progress`,
  `oracle-rich`, `oracle-with-separate-prompts`) encode `oracle.<verb>.*`, a
  shape the engine never produced. They are migrated to `oracle.call.*` (and
  `oracle-rich` keeps exercising all five verbs via `attrs.verb`). No dual-scheme
  support is added — the fiction is removed.
- **Real traces/cassettes** already emit `oracle.call.*`; nothing on disk needs a
  compat shim.
...
- `bugfix.spec.ts`: the 3 oracle assertions go green; add invariants — (a) each
  oracle call renders **exactly one** merged row, (b) each phase header appears
...
- [x] 1.1 Stamp oracle.call.start/complete with the foreground turn; verify state_path is the destination phase
...
- [x] 1.4 Guarantee a prompt reference on every oracle.call.start (inline or prompt_file); retire dead LLMCalled="oracle.ask.start". NB: the *cassette* emitter (`internal/testrunner/cassette.go` `writeCassetteOracleEvents`), not the live handlers, emits the start event during fixture replay — it was only offloading large prompts to sidecars and dropping small inline ones. Fixed to mirror the live dispatch path.
- [x] 1.5 Replay stays byte-identical for non-oracle events (Layer-7 check); regenerate bugfix fixture

## 2. Consumer — faithful representation
- [x] 2.1 Match oracle.call.start/complete, verb from attrs.verb (TraceTimeline, EventDetail, StateDiagram, oracle/*). Merged row stitches start's prompt + complete's response into one logical call (TraceTimeline `OracleMerge.merged`).
- [x] 2.2 Delete repairOracleTurns (snapshot-source.ts); snapshot == on-disk trace
...
- [x] 2.5 Bundle oracle-prompts sidecars into the artifact (build-artifact.mjs inlines prompt_file → prompt); usePromptLoader resolves under file://
- [x] 2.6 TaskDetail renders `response.text` for task-verb responses (was an empty Overview tab)

## 3. Prove
- [x] 3.1 Migrate the 5 synthetic fixtures to oracle.call.* (oracle-rich keeps all 5 verbs)
- [x] 3.2 bugfix.spec.ts: 9/9 green (the 3 oracle tests fixed); all six fidelity invariants verified by an independent render check (each aspect once, no same-column dup, prompt+response in every oracle row)
- [x] 3.3 Re-render bugfix artifact and eyeball both columns: each aspect once, no dup
- [ ] 3.4 REMAINING: update the *sibling-fixture* specs to the new inline-expansion model. `artifact.spec.ts` (×5: in-progress/completed/edge-cases) and `oracle-prompts.spec.ts` (×1) still assert the removed `.detail-drawer` UX and a now-dropped `machine.transition` row. These are obsolete-UX assertions broken by the §2 consumer rewrite — not trace-fidelity regressions — and need migrating to `.trace-timeline__row-body` inline expansion.

## 4. Document
- [x] 4.1 Update docs/tracing/trace-format.md (oracle.call.*, machine.say, intent events)
...
3. **Prompt delivery in artifacts** — inline prompts/responses into the snapshot
   ...
   offload threshold in `oracle_event_sink.go`.*
...
- **New oracle verbs or schema changes** to the oracle payloads themselves.
