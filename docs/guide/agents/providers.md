...
> **provider** retargets the `claude` subprocess behind an agent invocation —
> pointing it at an alternate Anthropic-compatible backend — without changing
> which agent verb runs or how its output is validated. It is orthogonal to
> the [agent plugin](../../architecture/agent-plugin.md) mechanism: plugins choose *which
> component answers*; providers choose *which backend the claude component
> talks to*.
>
> It is also orthogonal to the [agent **backend**](./backends.md)
> switch (`--agent claude|copilot`): the backend chooses *which coding-agent
...
Kitsoki's default agent verbs (`host.agent.{decide,ask,task,extract,converse,ask_with_mcp}`)
...
> mirrors the `agent_plugins:` env contract.
...
1. **Effect arg** — `with: { provider: <name> }` on a `host.agent.*` effect.
...
- **Orthogonal to plugins.** Providers affect the built-in claude path
  (`agent.claude` and any invocation that forks claude). They do not change
  `subprocess` / `mcp_http` / `builtin.local_llm` agent plugins, which carry
  their own transport config under `agent_plugins:`.
...
calls hit your normal Anthropic backend with Opus. Both run the same agent
