#!/usr/bin/env bash
# Convert a Playwright-recorded .webm into a high-quality looping GIF for
# embedding in PRs / markdown / chat. Uses the two-pass palettegen+paletteuse
# pipeline (a naive single-pass GIF looks muddy and banded).
#
# GIFs are large — keep --width modest (<=900) and --fps low (10-15). For
# anything long or detailed, prefer the MP4 (webm-to-mp4.sh).
#
# Usage: webm-to-gif.sh <input.webm> [output.gif] [--fps N] [--width W]
#   --fps N     frame rate (default 12)
#   --width W   output width px, aspect preserved (default 900)
set -euo pipefail

in="${1:?usage: webm-to-gif.sh <input.webm> [output.gif] [--fps N] [--width W]}"
shift || true
