...
- **Improve reports close the loop.** When a live session reaches a terminal
  ...
  **Auto-run at completion**, and **File evidence report** controls. The button
  ...
  This is also a first-class product-site feature:
  [Meta improvement reports](/features/meta-improvement.html) is captured by
  `tests/playwright/meta-improve-video.spec.ts` and generated by the feature
  catalog as `features/meta-improvement.yaml`.
