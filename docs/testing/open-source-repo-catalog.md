...
| `gears-rust` | Local/private Rust monorepo reference | Per-bug Cargo oracles; `BUGFIX_BAKEOFF_REPO=... make gears-bakeoff`; `BUGFIX_BAKEOFF_REPO=... make gears-history-full-smoke`; `suite: false` because the full workspace is heavy | Manifested with 4 armable bugs and additional reference-only cases | All four armable fixtures are RED->GREEN and pass the repo-history full-smoke path. The full-matrix `repo-bakeoff` flow renders the live commands and stops before cost-bearing model work. Not an OSS reuse candidate. |
...
| `BUGFIX_BAKEOFF_REPO=... make gears-bakeoff` | Free, gated, local heavyweight | Rust oracle path works against a local mirror without dirtying the checkout. | Private/local reference only. |
