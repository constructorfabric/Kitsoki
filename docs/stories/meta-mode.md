...
| `/work [--all]`          | on-path or meta      | Prints active async work. Without flags it is scoped to the current Kitsoki session: unread notifications, running/awaiting/failed jobs, queued chat drives, and background Claude chats. `--all` also includes queued chat drives and background Claude chats across every Kitsoki session on this host and seeds the same numbering cache as `/sessions list`, so `/sessions attach <N>` can switch directly to a background chat. |
...
- **`/attach` hands the terminal to claude.** While `/meta`
  ...
  `/work --all` lists queued/background claude work alongside the
  current session's other async work, and `/sessions list` gives the
  full active-session table. Both seed `/sessions attach <N>`, so you
  can hop between any active claude sessions — meta or otherwise — by
